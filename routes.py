:31
 flask import Blueprint, render_template, redirect, url_for, request, jsonify, make_response
...
def calculate_max_grade(student, target_lesson, all_lessons):
    ...
    for i in range(target_idx + 1, len(all_lessons)):
        next_l = all_lessons[i]
            ...
            mark = next((m for m in student.grades if m.lesson_id == next_l.id), None)                        if mark and mark.status == 'sick': continue
